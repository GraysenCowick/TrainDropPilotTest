...
export async function POST(request: NextRequest) {
  ...
  const ext = (filename as string).split(".").pop()?.toLowerCase() || "pdf";
  const filePath = `documents/${user.id}/${randomUUID()}.${ext}`;
  ...
  if (error || !data?.signedUrl) {
    console.error("[upload-document] createSignedUploadUrl failed:", error?.message);
