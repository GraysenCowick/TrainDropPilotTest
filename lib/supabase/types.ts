...
/*
  ...
  -- Migrations for existing modules table:
  -- ALTER TABLE modules ADD COLUMN IF NOT EXISTS description TEXT;

  -- Quiz questions (generated by Claude after transcription)
  CREATE TABLE quiz_questions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    module_id UUID NOT NULL REFERENCES modules(id) ON DELETE CASCADE,
    chapter_index INTEGER NOT NULL, -- -1 = final test, 0+ = chapter index
  ...
  CREATE POLICY "Module owners can view quiz_questions" ON quiz_questions FOR SELECT
    USING (EXISTS (SELECT 1 FROM modules WHERE modules.id = quiz_questions.module_id AND modules.user_id = auth.uid()));
  CREATE POLICY "Anyone can read published quiz_questions" ON quiz_questions FOR SELECT
    USING (EXISTS (SELECT 1 FROM modules WHERE modules.id = quiz_questions.module_id AND modules.status = 'published'));

  -- Quiz responses (submitted by employees)
  CREATE TABLE quiz_responses (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    module_completion_id UUID NOT NULL REFERENCES module_completions(id) ON DELETE CASCADE,
    question_id UUID NOT NULL REFERENCES quiz_questions(id) ON DELETE CASCADE,
    selected_answer INTEGER NOT NULL,
    is_correct BOOLEAN NOT NULL,
    answered_at TIMESTAMPTZ DEFAULT now(),
    UNIQUE(module_completion_id, question_id)
  );

  ALTER TABLE quiz_responses ENABLE ROW LEVEL SECURITY;
  CREATE POLICY "Anyone can insert quiz_responses" ON quiz_responses FOR INSERT WITH CHECK (true);
  CREATE POLICY "Anyone can update quiz_responses" ON quiz_responses FOR UPDATE USING (true);
  CREATE POLICY "Module owners can view quiz_responses" ON quiz_responses FOR SELECT
    USING (EXISTS (
      SELECT 1 FROM module_completions mc
      JOIN modules m ON m.id = mc.module_id
      WHERE mc.id = quiz_responses.module_completion_id
